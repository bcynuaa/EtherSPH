...
mutable struct LiquidThermalParticleMTh{
    RealType <: AbstractFloat, 
    ArrayType <: AbstractVector{RealType}, 
    AtomArrayType <: AbstractVector{Base.Threads.Atomic{RealType}},
} <: LiquidParticle
...
function LiquidThermalParticleMTh(RealType::DataType, dim::IntType)::LiquidThermalParticleMTh where IntType <: Integer
    x_vec = zeros(RealType, dim);
    v_vec = zeros(RealType, dim);
    dv_vec = [Base.Threads.Atomic{RealType}(0.) for i in 1: dim];
    drho = Base.Threads.Atomic{RealType}(0.);
    ...
    gap::RealType = 0.;
    t::RealType = 0.;
    dt = Base.Threads.Atomic{RealType}(0.);
    kappa::RealType = 0.;
    LiquidThermalParticleMTh(
        x_vec, v_vec, dv_vec, 
        rho, drho, 
        p, c, mass, gap, 
        sum_kernel_weight, 
