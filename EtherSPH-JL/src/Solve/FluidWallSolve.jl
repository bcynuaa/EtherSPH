#=
 ...
 function eachStep!(
    ...
    Threads.@threads for neighbour in f_w_neighbours
        wallForce!(fluid_particles[neighbour.i_], wall_particles[neighbour.j_], neighbour, smooth_kernel);
        # viscosityForce!(fluid_particles[neighbour.i_], neighbour, smooth_kernel, wc_lm);
        viscosityForce!(fluid_particles[neighbour.i_], wall_particles[neighbour.j_], neighbour, smooth_kernel, wc_lm);
    end
    Threads.@threads for i_f_particle in eachindex(fluid_particles)
...
function solve!(
    ...
    for step in ProgressBar(1: dr_forward_euler.total_step_)
        # eachStep!(fluid_particles, wall_particles, smooth_kernel, wc_lm, dr_forward_euler, step, check);
        try
            eachStep!(fluid_particles, wall_particles, smooth_kernel, wc_lm, dr_forward_euler, step, check);
        catch e
            println(e);
            println("step: ", step);
            writeVTP(step, step * dr_forward_euler.dt_, vtp_io, [fluid_particles, wall_particles]);
            break;
        end
